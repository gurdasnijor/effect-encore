...
const workerIdForOptions = (options: WorkflowEngineDurableStateOptions): string =>
...
> =>
  Effect.gen(function* () {
    ...
    return yield* makeWorkflowEngine(table, workerIdForOptions(options));
  }).pipe(Effect.provide(WorkflowEngineTable.layer(workflowEngineTableLayerOptions(options))));
...
const layer = (
  ...
  Layer.effectContext(
    Effect.gen(function* () {
      ...
      // workflow-engine-durable-state.ENGINE.5
      const table = yield* WorkflowEngineTable;
      const engine = yield* makeWorkflowEngine(table, workerIdForOptions(options));
      return Context.make(WorkflowEngineTable, table).pipe(
        Context.add(WorkflowEngine.WorkflowEngine, engine),
      );
    }),
  ).pipe(Layer.provide(WorkflowEngineTable.layer(workflowEngineTableLayerOptions(options))));
