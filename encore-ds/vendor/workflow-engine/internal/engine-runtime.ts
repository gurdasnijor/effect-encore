...
const orDieTable = <A>(effect: Effect.Effect<A, DurableTableError>): Effect.Effect<A> =>
...
export const makeWorkflowEngine = (
  ...
  Effect.gen(function* () {
    ...
    const fireClockWakeup = (row: WorkflowClockWakeupRow) =>
      Effect.gen(function* () {
        const current = yield* orDieTable(
          table.clockWakeups.get(row.clockKey).pipe(Effect.map(Option.getOrUndefined)),
        );
        if (!current || current.status !== "pending") return;
        yield* orDieTable(
          table.clockWakeups.upsert({
            ...current,
            status: "fired",
          }),
        );
    ...
    const recoverPendingClockWakeups = Effect.gen(function* () {
      const pending = yield* orDieTable(
        table.clockWakeups.query((coll) => coll.toArray.filter((row) => row.status === "pending")),
      );
      let index = 0;
    ...
    const isExecutionInterrupted = (executionId: string) =>
      orDieTable(
        table.executions
          .get(executionId)
          .pipe(Effect.map((row) => Option.getOrUndefined(row)?.interrupted === true)),
      );

    const getExecutionRow = (
      executionId: string,
    ): Effect.Effect<WorkflowExecutionRow | undefined> =>
      orDieTable(table.executions.get(executionId).pipe(Effect.map(Option.getOrUndefined)));
    ...
    const resume = (executionId: string) =>
      Effect.gen(function* () {
        ...
        Object.assign(instance, {
          ...
          cause: row.cause as typeof instance.cause,
        });
        ...
        const executeEffect: Effect.Effect<
          ...
          unknown,
          WorkflowEngine.WorkflowEngine | WorkflowEngine.WorkflowInstance
        > = row.interrupted ? Effect.interrupt : entry.execute(row.payload as object, executionId);
            ...
            Effect.gen(function* () {
              ...
              yield* orDieTable(
                table.executions.upsert({
                  ...
                  ...(result._tag === "Suspended" && result.cause !== undefined
                    ? { cause: result.cause }
                    : {}),
    ...
    const recoverPendingDeferreds = (workflowName: string) =>
      Effect.gen(function* () {
        const deferredRows = yield* orDieTable(
    ...
    const engine = WorkflowEngine.makeUnsafe({
        ...
        Effect.gen(function* () {
          ...
          Effect.withSpan("firegrid.workflow_engine.execution.execute", {
            ...
            attributes: {
              ...
              "firegrid.workflow.name": workflow.name,
              "firegrid.workflow.discard":  options.discard,
            },
          }),
      ...
      interrupt: (_workflow, executionId) => interruptExecution(_workflow, executionId),
      ...
      interruptUnsafe: (_workflow, executionId) => interruptExecution(_workflow, executionId),
      ...
      activityExecute: (activity, attempt) =>
        ...
        Effect.andThen(
          ...
          Effect.gen(function* () {
            ...
            const row = yield* orDieTable(
              table.activities.get(activityKey).pipe(Effect.map(Option.getOrUndefined)),
            );
            ...
            const completedAfterClaim = yield* orDieTable(
              table.activities.get(activityKey).pipe(Effect.map(Option.getOrUndefined)),
            );
            ...
            const result = yield* activity.executeEncoded.pipe(
              ...
              Effect.catchCause((cause: Cause.Cause<never>) =>
                Effect.gen(function* () {
                  const interrupted = yield* isExecutionInterrupted(instance.executionId);
            ...
            Effect.withSpan("firegrid.workflow_engine.activity.execute", {
              ...
              attributes: {
                ...
                "firegrid.contract.id":
                  "features/firegrid/workflow-engine-durable-state.feature.yaml",
              },
            }),
          ),
        ),
      deferredResult: (deferred) =>
        Effect.gen(function* () {
          ...
          const row = yield* orDieTable(
            table.deferreds.get(key).pipe(Effect.map(Option.getOrUndefined)),
          );
